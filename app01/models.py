...
class DegreeCourse(models.Model):
    ...
    # 用于GenericForeignKey反向查询，不会生成表字段，切勿删除
    degreecourse_price_policy = GenericRelation("PricePolicy")#关联PricePolicy表
...
class Course(models.Model):
    ...
    course_img = models.CharField(max_length=255)#课程图片

    sub_category = models.ForeignKey("CourseSubCategory",on_delete=models.CASCADE) #外键，是谁的子类

    course_type_choices = ((0, '付费'), (1, 'VIP专享'), (2, '学位课程'))#该课程的权限
    course_type = models.SmallIntegerField(choices=course_type_choices)#

    degree_course = models.ForeignKey("DegreeCourse", blank=True, null=True, help_text="若是学位课程，此处关联学位表",on_delete=models.CASCADE)#如果是学位表则关联学位表


    brief = models.TextField(verbose_name="课程（模块）概述", max_length=2048)#课程描述
    level_choices = ((0, '初级'), (1, '中级'), (2, '高级'))
    level = models.SmallIntegerField(choices=level_choices, default=1)#课程等级
    pub_date = models.DateField(verbose_name="发布日期", blank=True, null=True)#发布日期
    period = models.PositiveIntegerField(verbose_name="建议学习周期(days)", default=7)#学习周期
    order = models.IntegerField("课程顺序", help_text="从上一个课程数字往后排")
    attachment_path = models.CharField(max_length=128, verbose_name="课件路径", blank=True, null=True)#课件路径，可以从为空
    status_choices = ((0, '上线'), (1, '下线'), (2, '预上线'))#课程状态 是否上线
...
class CourseDetail(models.Model):
    """课程详情页内容"""
    course = models.OneToOneField("Course",on_delete=models.CASCADE)#连接课程表
    hours = models.IntegerField("课时")#课时
    course_slogan = models.CharField(max_length=125, blank=True, null=True)#学习目标
    video_brief_link = models.CharField(verbose_name='课程介绍', max_length=255, blank=True, null=True)#课程介绍
...
class PricePolicy(models.Model):
    ...
    valid_period = models.SmallIntegerField(choices=valid_period_choices)#这里面给的是周期
...
class Comment(models.Model):
    ...
    account = models.ForeignKey("Auth", verbose_name="会员名",on_delete=models.CASCADE)
    disagree_number = models.IntegerField(default=0, verbose_name="踩")
    agree_number = models.IntegerField(default=0, verbose_name="赞同数")
    date = models.DateTimeField(auto_now_add=True)
